...
class TemplateIncludeSystem {
    ...
    setActiveNavigation() {
        ...
        if (currentPath === '/' || currentPath === '/index.html' || currentPath === '' || currentPath.endsWith('index3.html')) {
...
function enableCrossPageNavigation() {
  document.addEventListener('includesLoaded', function() {
    ...
    var isMainPage = window.location.pathname.endsWith('index3.html') || window.location.pathname === '/' || window.location.pathname.endsWith('/index3.html');
    if (isMainPage) return;
    var navLinks = document.querySelectorAll('.nav-link');
    navLinks.forEach(function(link) {
      var href = link.getAttribute('href');
      if (href === '#hero') {
        link.setAttribute('href', 'index3.html');
        link.addEventListener('click', function(e) {
          // index3.html'e git
        });
      } else if (href && href.startsWith('#')) {
        link.setAttribute('href', 'index3.html' + href);
        link.addEventListener('click', function(e) {
          // index3.html#section'a git
        });
      }
    });
  });
